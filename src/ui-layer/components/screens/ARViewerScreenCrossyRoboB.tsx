...
// Utility function to detect Unity connections vs web viewers based on UID
const isUnityConnection = (uid: number): boolean => {
  // Web viewers use UIDs 1-99,999 (generated by Math.floor(Math.random() * 100000))
  // Unity apps use UIDs 100,000+ or other long number patterns
  return uid >= 100000;
};
    ...
    try {
      ...
      client.on('user-published', async (user, mediaType) => {
        ...
        if (client.connectionState !== 'CONNECTED') {
          console.log(`⚠️ Skipping subscription to ${mediaType} from user ${user.uid} - client not connected`);
          return;
        }
        ...
        // Minimal track logging for performance
        ...
        setRemoteUsers(prev => {
          ...
          if (mediaType === 'video' && user.videoTrack) {
            ...
            // Determine if this is the host - Only Unity connections can become hosts
            // Web viewers (UID < 100000) can never become the main host
            
            const isUnityUser = isUnityConnection(user.uid as number);
            const newMapSize = newMap.size;
            const isFirstRobot = !hostUser && newMapSize === 1;
            const isCurrentHost = hostUser && hostUser.uid === user.uid;
            
            // Only Unity connections can become host in Robot B interface:
            // 1. Must be a Unity connection (UID >= 100000), AND
            // 2. Either no current host OR this newer Unity joiner takes over
            // Web viewers are never considered for host role
            const shouldBeHost = isUnityUser && (!hostUser || true); // Unity connections can always take over
            
            // Reduced logging for performance
            if (shouldBeHost) {
              console.log(`👑 User ${user.uid} becoming host (Unity connection)`);
            }
            
            if (shouldBeHost) {
              console.log(`👑 User ${user.uid} is now the Crossy Robo HOST (Unity connection in Robot B interface)`);
              ...
              if (existingHostContainer) {
                ...
                const attemptVideoContainerCreation = (retryCount = 0) => {
                  const maxRetries = 50; // Try for 5 seconds (50 × 100ms) - increased timeout
                  ...
                  if (mainViewRef.current) {
                    ...
                    // Minimal video metadata logging for performance
                    hostVideo.addEventListener('loadedmetadata', () => {
                      if (hostVideo.videoHeight < 900) {
                        console.warn('⚠️ Low resolution detected:', hostVideo.videoHeight, 'pixels');
                      }
                    });
                    ...
                    try {
                      ...
                      console.log(`✅ Host video playing for user ${user.uid}`);
                    } catch (error) {
                    ...
                    // Reduced retry frequency for better performance
                    setTimeout(() => attemptVideoContainerCreation(retryCount + 1), 200);
                  } else {
                    console.error(`❌ mainViewRef unavailable after ${maxRetries} attempts`);
                  }
                };
                
                // Start the retry mechanism with a longer initial delay to ensure DOM is ready
                setTimeout(() => attemptVideoContainerCreation(), 500);
              }
            } else {
              const userType = isUnityUser ? 'Unity viewer' : 'Web viewer';
              console.log(`👥 User ${user.uid} is a ${userType} with video`);
      ...
      // Set client role based on connection type
      // Unity connections (UID >= 100000) join as host, web viewers join as audience
      const uid = Math.floor(Math.random() * 100000);
      const clientRole = isUnityConnection(uid) ? 'host' : 'audience';
      
      console.log(`Setting client role to '${clientRole}' for UID ${uid} (Unity: ${isUnityConnection(uid)})`);
      await client.setClientRole(clientRole);
      
      // UID already generated above for role determination
      setLocalUid(uid);
      
      // Join channel - HARDCODED FOR TESTING
      const channelName = 'robot-video'; // Hardcoded channel name
      const token = await fetchToken(channelName, uid, clientRole);
