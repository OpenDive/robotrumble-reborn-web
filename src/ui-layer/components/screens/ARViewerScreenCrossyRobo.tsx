...
export const ARViewerScreenCrossyRobo: React.FC<ARViewerScreenCrossyRoboProps> = ({ session, onBack }) => {
  ...
  const connectToAgoraStream = async () => {
    try {
      ...
      client.on('user-published', async (user, mediaType) => {
        ...
        setRemoteUsers(prev => {
          ...
          if (mediaType === 'video' && user.videoTrack) {
            ...
            // Only Unity connections can become host:
            // 1. Must be a Unity connection (UID >= 100000), AND
            // 2. Either first robot to connect OR already the current host
            // Web viewers are never considered for host role
            const shouldBeHost = isUnityUser && (isFirstRobot || isCurrentHost);
            ...
            if (shouldBeHost) {
              console.log(`👑 User ${user.uid} is now the Crossy Robo HOST (Unity connection with Robot A priority)`);
      ...
      // Generate UID and determine role
      // Web viewers use UIDs 1-99,999, Unity apps use UIDs 100,000+
      const uid = Math.floor(Math.random() * 100000);
      const isUnity = isUnityConnection(uid);
      const clientRole = isUnity ? 'host' : 'audience';
      
      console.log(`Setting client role to '${clientRole}' for UID ${uid} (Unity: ${isUnity})`);
