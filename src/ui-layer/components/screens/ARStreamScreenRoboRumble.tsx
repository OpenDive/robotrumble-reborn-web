...
export const ARStreamScreenRoboRumble: React.FC<ARStreamScreenRoboRumbleProps> = ({ session, onBack }) => {
  ...
  const HOST_PASSWORD = 'rumble2025'; // RoboRumble specific password
  ...
  const handlePasswordSubmit = (e: React.FormEvent) => {
    ...
    if (passwordInput === HOST_PASSWORD) {
      ...
      setPasswordError('');
      console.log('✅ RoboRumble Host authentication successful');
    } else {
      setPasswordError('Incorrect password. Please try again.');
      setPasswordInput('');
      console.log('❌ RoboRumble Host authentication failed');
    }
  };
      ...
      <div className="w-full h-screen bg-[#0B0B1A] relative overflow-hidden flex items-center justify-center">
        ...
        <div className="relative z-10 bg-black/60 backdrop-blur-sm border border-white/20 rounded-xl p-8 max-w-md w-full mx-4">
          <div className="text-center mb-6">
            ...
            <h2 className="text-2xl font-bold text-white mb-2">RoboRumble Host Access</h2>
            <p className="text-white/70">Enter the host password to control RoboRumble</p>
          </div>
          ...
          <form onSubmit={handlePasswordSubmit} className="space-y-4">
            <div>
              <input
                ...
                placeholder="Enter RoboRumble host password"
            ...
            <div className="flex gap-3">
              ...
              <button
                ...
                className="flex-1 px-4 py-3 bg-red-600 hover:bg-red-700 disabled:bg-gray-600 disabled:cursor-not-allowed text-white font-medium rounded-lg transition-colors"
              >
                Access RoboRumble Host
              </button>
            </div>
          </form>
          ...
          <div className="mt-6 pt-4 border-t border-white/10">
            <p className="text-xs text-white/50 text-center">
              RoboRumble Session: {session.trackName} • Channel: {session.id}
            </p>
          </div>
        </div>
      </div>
